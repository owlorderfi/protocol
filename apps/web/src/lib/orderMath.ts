...
// computeExpectedAmountOut + applySlippage are now canonical in
// @owlorderfi/shared (so the API can re-derive the same minAmountOut to
// enforce a slippage bound at creation). Re-exported here unchanged so the
// existing `from '../lib/orderMath'` imports across the create forms keep
// working. The volatility / slippage-suggestion helpers below stay local.
export { computeExpectedAmountOut, applySlippage, MAX_SLIPPAGE_BPS } from '@owlorderfi/shared';
